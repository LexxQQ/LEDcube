...
#define INVERT_X 1    // инвертировать по горизонтали (если текст не читается)

String disp_text = "EVA ALLA VITALIK"; // текст для режима текста
...
GButton butt1(BUTTON1, LOW_PULL, NORM_OPEN);
GButton butt2(BUTTON2, LOW_PULL, NORM_OPEN);
...
void loop() {
  ...
  //Serial.println(butt1.isHold());
  if (butt1.isHold() && tmrStep.isReady()) {                                 // если кнопка была удержана (это для инкремента)
    modeTimer += buttStep;               // увеличивать/уменьшать переменную value с шагом и интервалом
    if (modeTimer > 1000) {
      modeTimer = 1000;
    }
    //Serial.println(modeTimer);
  }
  if (butt2.isHold() && tmrStep.isReady()) {                                 // если кнопка была удержана (это для инкремента)
    modeTimer -= buttStep;               // увеличивать/уменьшать переменную value с шагом и интервалом
    if (modeTimer < 50) {
      modeTimer = 50;
    }
